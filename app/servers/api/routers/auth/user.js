...
module.exports = class extends router.Base {
  setup () {
    this.patch('/forget', async (ctx, next) => {
    ...
    this.get('/profile', user.required(), async ctx => { ctx.body = ctx.state.user })

    this.patch('/profile', user.required(), async ctx => {
