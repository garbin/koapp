...
exports.default = function server () {
  ...
  for (let app of config.universal.apps) {
    if (app.server) {
      universal.use(mount(app.point, require(`../../server`).default.koa))
      continue
    }
    if (app.client) {
    ...
    if (app.middleware) {
      universal.use(mount(app.point, require(`../../server/middlewares/${app.middleware}`).default))
      continue
    }
    universal.use(mount(app.point, require(app.path)))
  }
